...
- Out of our five models, we will generally recommend using the ensemble model as the accuracy score from cross_val_score is the highest among all the models used. However, if we aim to flag out the high risk individuals, we should use MLP model as it yields the highest recall score for people with cardio disease in our dataset, meaning we will be able to correctly flag out larger percentage of people who has high risk of cardio in the near future. 
